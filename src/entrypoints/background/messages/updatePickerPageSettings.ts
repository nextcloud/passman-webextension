...
import { PageRuleOverrideSelection, pageRuleOverrideSelectionToBoolean } from "@/lib/pageRules/pageRulesOverrides";
...
export interface UpdatePickerPageSettingsRequest {
     ...
     */
    updatedPageRules: Partial<Record<keyof PageRulesInterface, PageRuleOverrideSelection>>;
}
...
onMessage('updatePickerPageSettings', async (request) => {
    ...
    await PageRulesService.setPageRules(
        ...
        Object.fromEntries(Object.entries(request.data.updatedPageRules).map(
            ([key, value]) => [key, pageRuleOverrideSelectionToBoolean(value)]
        ))
    );
});
